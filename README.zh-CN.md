...
| `maxInputChars` | `12000` | 输入字数上限(按 Unicode 码点统计,一个 emoji 算一个字);超限**拒绝而不截断** |
...
| `maxConcurrent` | `2` | 宿主侧并发上限;超出的请求返回 `429` |
| `rateLimitPerMinute` | `10` | 每分钟滑动窗口限流;超出的请求返回 `429` |
...
- **滥用上限** —— `Origin` 门拒绝来自非本地页面的浏览器调用;路由还执行并发上限(`maxConcurrent`,默认 2)与每分钟限流(`rateLimitPerMinute`,默认 10),超限返回 `429`。
